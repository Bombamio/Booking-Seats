"""CRUD-слой пользователей.

Модуль описывает операции чтения и записи для модели `User`.

Классы:
   - `CRUDUser` — проверка контактов, создание с хешем пароля,
     привязка менеджеров к кафе.

Связанные слои:
   - бизнес-логика — в `src/services/user.py`.
"""
